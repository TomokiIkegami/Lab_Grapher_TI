...
## 横軸と縦軸の表示範囲を設定
ax.set_xlim(0,100)   # 必要に応じて最大、最小値を設定
ax.set_ylim(-50,50)  # 必要に応じて最大、最小値を設定
...
ax.set_xlabel("$x$ [{0}m]".format(aux_unit_x),fontsize=12,fontname="Times New Roman")    # 軸ラベルを英語で入力。$マークに挟まれている範囲は数式が入力可能。ここでもフォントサイズの設定が可能。
ax.set_ylabel("$y$ [{0}m]".format(aux_unit_y),fontsize=12,fontname="Times New Roman") # 軸ラベルを英語で入力。$マークに挟まれている範囲は数式が入力可能。ここでもフォントサイズの設定が可能。
