...
# ## マーカー（データの数より多く用意）
point_mk = ["o", "s", "^", "x", "o", "o", "o", "o", "None"]  # マーカーの設定（必要に応じて）
...
for i in range(len(df.columns.values[1:])):
    ...
    ax.scatter(x=df4.iloc[:,0].to_list(),y=df4.iloc[:,i+1].to_list(),s=2,marker=point_mk[i])  # 散布図のみならこの行をだけ有効に（s:マーカーサイズ、marker:マーカ種類）
