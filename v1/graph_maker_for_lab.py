...
## 横軸と縦軸の表示範囲を設定
plt.xlim(0,1e-02)   # 必要に応じて最大、最小値を設定
plt.ylim(0,0.2e+9)  # 必要に応じて最大、最小値を設定

## 軸ラベルの設定。補助単位も自動で設定する。
...
ax.set_xlabel("Time $t\,$ [{0}s]".format(aux_unit_x),fontsize=12,fontname="Times New Roman")    # 軸ラベルを英語で入力。$マークに挟まれている範囲は数式が入力可能。ここでもフォントサイズの設定が可能。
ax.set_ylabel("Stress $\sigma\,$[{0}Pa]".format(aux_unit_y),fontsize=12,fontname="Times New Roman") # 軸ラベルを英語で入力。$マークに挟まれている範囲は数式が入力可能。ここでもフォントサイズの設定が可能。
...
# 凡例を追加
labels=["min","max","average"]  # 入力データに合わせて設定
